...
from entrycounter.peopletracker import PeopleTracker
from entrycounter.tripline import Tripline
...
class EntryCounter:
    ...
    def process(self, frame):
        ...
        def draw(event,x,y,flags,param):
            ...
            if event == cv2.EVENT_LBUTTONDOWN:
                ...
                if(count == 1):
                    ...
                    points.append((x,y))
                    print(points)
                    config.set("triplines","line1_start",str(points[0]))
                    config.set("triplines","line1_end",str(points[1]))
                    op = open(path,'w')
                    config.write(op)
                    op.close()
                    #run.start()
        ...
        if self.show_window:
            cv2.imshow('EntryCounter', frame)
            cv2.setMouseCallback('EntryCounter',draw)
    ...
    def new_collision(self, person):
        ...
        if not self.to_stdout:
            print("%s is heading %s." % (person.name, person.meta['line-0']))
